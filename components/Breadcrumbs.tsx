            ...
            {displayBreadcrumbs.map((crumb, index) => {
              ...
              if (isEllipsis) {
                return (
                  <li key="ellipsis" className="relative group">
                    ...
                    {showEllipsis && (
                      <div className="absolute left-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-50 min-w-max">
                        {hiddenBreadcrumbs.map((item, idx) => (
                          <Link
                            key={item.path}
                            href={item.path}
                            className="block px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 transition-colors first:rounded-t-lg last:rounded-b-lg text-sm"
              ...
              return (
                <li key={crumb.path} className="flex items-center min-w-0">
                  {index > 0 && (
                    <span className="text-gray-300 dark:text-gray-600 mx-1.5 shrink-0" aria-hidden="true">
                      /
                    </span>
                  )}
