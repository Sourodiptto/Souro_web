            ...
            {displayBreadcrumbs.map((crumb, index) => {
              ...
              if (isEllipsis) {
                return (
                  <li key="ellipsis" className="relative group">
                    ...
                    {showEllipsis && (
                      <div className="absolute left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-max">
                        {hiddenBreadcrumbs.map((item, idx) => (
                          <Link
                            key={item.path}
                            href={item.path}
                            className="block px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-colors first:rounded-t-lg last:rounded-b-lg text-sm"
              ...
              return (
                <li key={crumb.path} className="flex items-center min-w-0">
                  {index > 0 && (
                    <span className="text-gray-300 mx-1.5 shrink-0" aria-hidden="true">
                      /
                    </span>
                  )}
                  ...
                  {isActive ? (
                    ...
                    <Link
                      href={crumb.path}
                      className="text-gray-600 hover:text-gray-900 transition-all duration-200 relative group truncate"
                    >
                      {crumb.label}
